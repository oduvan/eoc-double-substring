...
requirejs(['ext_editor_io', 'jquery_190'],
    function (extIO, $) {
        ...
        function doubleSubstringCanvas(dom, input, answer) {
            ...
            const reg = RegExp( '(' + '.'.repeat(answer) + ').*\\1' );
            const mt = input.match(reg);
            const indexs = make_idxs(mt[1]);
        ...
        var io = new extIO({
            ...
            animation: function($expl, data){
                doubleSubstringCanvas($expl, data.in, data.ext.answer);
            }
        });
        io.start();
    }
